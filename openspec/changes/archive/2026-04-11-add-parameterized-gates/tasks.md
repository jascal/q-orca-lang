...
- [x] 1.1 Add an `_evaluate_angle(text: str) -> float` helper in
...
- [x] 1.2 Extend `_parse_gate_from_effect` to match
      `R([XYZ])\(\s*\w+\[(\d+)\]\s*,\s*([^)]+)\s*\)` and construct
      `QuantumGate(kind="R{X|Y|Z}", targets=[N], parameter=<angle>)`.
- [x] 1.3 Surface parser errors on malformed rotation-gate effects
...
- [x] 3.1 Add parametrized parser tests in `tests/test_parser.py`
      covering each angle grammar form plus a bare-identifier
      error case.
- [x] 3.2 Add a regression test in `tests/test_regression.py` that
...
- [x] 3.3 Create `tests/test_vqe_rotation.py` that runs the new example
...
- [x] 4.1 Create `examples/vqe-rotation.q.orca.md`: a single-qubit
...
- [x] 4.2 Wire the new example into `tests/test_examples.py` so it
      runs under the standard example harness.

## 5. Documentation

- [x] 5.1 Update the README roadmap to mark parameterized gates as
      shipped, cross-linked to the CHANGELOG entry.
- [x] 5.2 Update `openspec/specs/language/spec.md` and
...
- [x] 5.3 Add a CHANGELOG entry under the next version bump (0.3.3 or

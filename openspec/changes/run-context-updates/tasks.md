...
- [x] 4.1 Create `q_orca/runtime/iterative.py` with
  `simulate_iterative(machine: QMachineDef,
  options: QIterativeSimulationOptions) ->
  QIterativeSimulationResult`.
- [x] 4.2 Initialize a mutable context record from
  `machine.context` defaults and an empty `bits` dict.
- [x] 4.3 Walk transitions from the initial state: enumerate
...
- [x] 4.4 Dispatch on action kind: gate/effect → build a per-segment
...
- [x] 5.3 Update the file-level banner logic: if the machine has
  ...
  unchanged.
- [x] 5.4 CUDA-Q and cuQuantum banner additions: when a machine
...
- [x] 6.1 In `q_orca/runtime/python.py::simulate_machine`, detect
...
- [x] 6.2 Keep the existing flat-circuit path unchanged for every
...
- [x] 6.3 Wire `simulate_iterative` into `q-orca verify --run`
...
- [x] 7.1 In `q_orca/verifier/classical_context.py`, add
...
- [x] 7.2 Respect the existing `VerifyOptions.skip_classical_context`
  skip flag.
- [x] 7.3 Unit tests: QPC learning example (no warning — has
...
- [x] 8.1 Delta `openspec/changes/run-context-updates/specs/
...
- [x] 8.2 Delta `openspec/changes/run-context-updates/specs/
...
- [x] 8.4 Update `README.md` roadmap: move the line for
  `add-classical-context-updates` to reflect full shipment,
  and add this change under Recently shipped on landing.
- [x] 8.5 Update `CHANGELOG.md` with the behavioral-change note
  from design §Migration Plan.
- [x] 8.6 Run `openspec validate run-context-updates --strict`
  before opening the PR.

## 9. End-to-end verification

- [x] 9.1 Extend `examples/predictive-coder-minimal.q.orca.md`
...
- [x] 9.2 Confirm `q-orca verify --strict` returns VALID on the
  example with no `UNBOUNDED_CONTEXT_LOOP` warning.
- [x] 9.3 Confirm `q-orca verify --backend qiskit --run` drives

<!--
v1 scope = the Stage-4b VERIFICATION fast path (Clifford classifier, tableau
entanglement, stim backend, auto-routing, force/refuse). Tasks marked
"(DEFERRED → follow-on)" are the SAMPLING sub-feature — compile_to_stim, the
Aer-stabilizer engine, the shots-based distribution-parity test, and the two
authored QEC examples. They exercise a path verify() does not use; the
intractability win is demonstrated instead by a 30-qubit-GHZ stim test, and the
entanglement computation is validated directly against QuTiP. See design.md.
-->
...
- [x] 1.1 Add a `stabilizer` extras group to `pyproject.toml` pulling in
  ...
  `qiskit-aer` unpinned).
- [x] 1.2 Detect `stim` and the Aer stabilizer method at module load (mirror the
...
- [x] 2.3 Add `NON_CLIFFORD_GATE_IN_STABILIZER_BACKEND` to the verifier error
  codes (a string `code` on `QVerificationError` — no enum to extend).

## 3. Stabilizer compilation targets (compiler)

- [ ] (DEFERRED → follow-on) 3.1 `compile_to_stim(machine) -> stim.Circuit`: map Clifford gates to Stim
...
- [ ] (DEFERRED → follow-on) 3.2 `compile_to_qiskit_stabilizer(machine) -> QuantumCircuit`: reuse
...
- [x] 4.1 New `q_orca/backends/stim_backend.py` implementing `BackendAdapter`
...
- [ ] (DEFERRED → follow-on) 4.2 New `q_orca/backends/qiskit_stabilizer_backend.py` wrapping
  `AerSimulator(method="stabilizer")`.
- [x] 4.3 Register both in `q_orca/backends/__init__.py`; add the `stabilizer`
...
- [x] 5.3 CLI: accept `stabilizer` / `stim` (and document `auto` as the default)
  ...
  `--backend` help text.
- [x] 5.4 Force-on-non-Clifford: raise `NON_CLIFFORD_GATE_IN_STABILIZER_BACKEND`
  (fatal) unless `stabilizer_fallback: state-vector`, which warns and uses the
  state-vector path.

## 6. Verifier — native entanglement

- [x] 6.1 Under the stabilizer backend, evaluate entanglement / `schmidt_rank(…)`
...
- [ ] (DEFERRED → follow-on) 7.1 `examples/surface-code-3.q.orca.md`: one round of the distance-3
  rotated surface code (17 physical qubits — intractable for state-vector).
- [ ] (DEFERRED → follow-on) 7.2 `examples/bit-flip-repeated.q.orca.md`: three rounds of the 3-qubit
...
- [ ] (DEFERRED → follow-on) 8.3 Distribution parity: `active-teleportation` outcome distribution under
  the stabilizer backend matches QuTiP within statistical tolerance at
  `shots=10000` (seeded).
- [x] 8.4 `NON_CLIFFORD_GATE_IN_STABILIZER_BACKEND` fires on forced stabilizer +
  non-Clifford machine; `stabilizer_fallback: state-vector` downgrades to a warning.
- [x] 8.5 A `schmidt_rank(q0,q1) >= 2` invariant on a Bell machine is satisfied
...
- [x] 8.6 Auto-routing: Clifford machine selects stabilizer when available, falls
  back to state-vector (with warning) when Stim/Aer absent.
- [x] 8.7 `surface-code-3` verifies on the stabilizer path within a CI time bound.
- [x] 8.8 Angle-fold edge case: pin which rotation angles the `angle.py` evaluator
...
- [x] 9.1 Document the stabilizer backend in `docs/language/`: backend selection
...
- [x] 9.2 Mark `docs/research/spec-stabilizer-fast-path-backend.md` delivered,

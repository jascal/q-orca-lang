...
- [ ] 1.1 Pin the new encoding shape to **Alternative A**
...
- [ ] 1.2 Run an angle-design pass for 12 concepts under the new
...
- [ ] 1.3 Record the achieved per-tier bands (exact numerical
      values from the angle-design pass) here, so downstream tasks
      can re-pin against them.

- [ ] 1.4 Re-derive the polysemy column for `|f⟩ = |c_0⟩` (the
...
- [ ] 2.1 In `q_orca/compiler/concept_gram_mps.py`, generalize the
...
- [ ] 2.4 Verify that the strict single-bound-param staircase
...
- [ ] 3.1 Rewrite the `prepare_concept` and `query_concept` effect
...
- [ ] 3.2 Re-pin the leading paragraph: drop the false claim that
...
- [ ] 3.3 Re-pin the analytic per-tier band table to the bands
      from task 1.3.

- [ ] 3.4 Re-pin the ASCII Gram heatmap to the new bands. The
      heatmap's four-tier visual structure is preserved; only the
      cell values shift slightly.

- [ ] 3.5 Re-pin the polysemy column table for `|f⟩ = |dog⟩`
      from task 1.4.

- [ ] 3.6 Adjust the 12 concept (α, β, γ) triples if task 1.2's
      angle pass selected a tweaked design. Otherwise leave them
      as the archived values.

## 4. Update the demo

- [ ] 4.1 In `demos/larql_polysemantic_hierarchical/demo.py`,
      re-pin the §3 analytic Gram heatmap to the new bands.

- [ ] 4.2 In §6 (rung-0 vs. rung-1 recap), reword the narrative.
...
- [ ] 4.3 If the demo prints the analytic vs. simulated Gram
...
- [ ] 5.1 In `README.md`, find the "Hierarchical polysemy" section
...
- [ ] 5.2 Append a short post-mortem section (~10 lines) at the
...
- [ ] 5.3 In `docs/research/polysemantic-encoding-beyond-product-
...
- [ ] 5.4 Update the rung-1 entry in the same research note to
...
- [ ] 6.2 Add a non-factorization assertion to the same test:
...
- [ ] 6.3 Extend `tests/test_compiler.py::TestComputeConceptGramMps`
...
- [ ] 6.4 Add a sad-path test for the new
...
- [ ] 8.1 Run `openspec validate fix-mps-encoding-non-factorizing
...
- [ ] 8.2 Run `openspec list` and verify the change appears as

...
- [ ] 1.1 Create `q_orca/effect_parser.py` with `ParsedGate` dataclass
      (`name: str, targets: tuple[int, ...], controls: tuple[int, ...],
      parameter: float | None`).
- [ ] 1.2 Implement `parse_single_gate(effect_str, angle_context) ->
...
- [ ] 1.3 Implement `parse_effect_string(effect_str, angle_context) ->
      list[ParsedGate]` splitting on `;` and calling
      `parse_single_gate` per part.
- [ ] 1.4 Every gate kind currently recognized by any of the three
...
- [ ] 3.2 Rewrite `_parse_effect_to_gate_dicts` as a thin adapter over
      `parse_effect_string`.
- [ ] 3.3 Delete the verifier's regex block and the TODO comment at
      `_parse_single_gate_to_dict`.
- [ ] 3.4 Extend `tests/test_verifier.py::TestContextAngleDynamicVerifier`
      with a parametrize over the shared fixture covering every gate
      kind's dict-shape output.
- [ ] 3.5 Verify the suite still passes (currently 406 tests).

## 4. Migrate the Qiskit compiler
- [ ] 4.1 Rewrite `q_orca/compiler/qiskit.py::_parse_single_gate` as a
      thin adapter over `parse_single_gate` building `QuantumGate`.
- [ ] 4.2 Rewrite `_parse_effect_string` as a thin adapter over
      `parse_effect_string`.
- [ ] 4.3 Verify `q_orca/compiler/qasm.py` still works via its current
      import path, or update it to import from `q_orca/effect_parser.py`
      directly if cleaner.
- [ ] 4.4 Extend `tests/test_compiler.py` with a parametrize over the
      shared fixture covering both Qiskit and QASM emission for every
      gate kind.

## 5. Migrate the markdown parser
- [ ] 5.1 Rewrite `q_orca/parser/markdown_parser.py::_parse_gate_from_effect`
...
- [ ] 5.2 Remove any redundant effect-parsing regexes from the markdown
      parser.
- [ ] 5.3 Extend `tests/test_parser.py` with a parametrize over the
      shared fixture asserting `QuantumGate` AST shape.

## 6. Cleanup & documentation
- [ ] 6.1 Delete the now-dead effect-parsing code and any imports that
      are no longer used in the three call-site modules.
- [ ] 6.2 Update `openspec/specs/compiler/spec.md` to state that the
      shared parser is the single source of truth for effect-string
      gate parsing.
- [ ] 6.3 Update `openspec/specs/verifier/spec.md` to reference the
      shared parser.

## 7. Spec consistency
- [ ] 7.1 `openspec validate consolidate-gate-parser --strict` is green.
- [ ] 7.2 Full pytest suite green (≥ 406 passed, 4 skipped).
- [ ] 7.3 Ruff clean across the touched files.

## 8. Follow-up verification
- [ ] 8.1 Manually run the pre-PR-#11 regression scenarios: QAOA example
...
- [ ] 8.2 Add `CCNOT(qs[0], qs[1], qs[2])` as a positive test case to

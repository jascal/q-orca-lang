...
6. ✅ **Delivered** (`add-qpc-convergence-benchmark`) — the QPC learning loop is now demonstrated to converge. `examples/predictive-coder-converging.q.orca.md` is a real learning machine (data register `|0>` so the parity error `sin^2(theta_0/2)` depends on `theta_0`; a real `gradient_step`; the model re-prepared each iteration via a `|ready>` re-entry state), and `q_orca/evaluation/qpc.py` runs it through the iterative runtime, measures the per-iteration error trajectory, compares it against the exact classical baseline `p(theta_0)=sin^2(theta_0/2)`, and certifies convergence to `theta_0* = pi/2` within an `O(eta)` band (see `demos/qpc_convergence/`). Building this surfaced — and fixed — two real gaps the original `predictive-coder-learning` skeleton hid: that example has *no learning signal* (data `|+>` ⇒ `P(error)=1/2` ∀θ) *and* its `gradient_step` is a no-op; and the verifier rejected scalar-`float` context mutation (so a learnable bare-scalar angle could not be both rotation-gate-referenced and updated). The benchmark uses the exact classical-gradient-descent baseline only; the 2202.01230 autoencoder-compression comparison on sequential quantum data remains a larger, separate effort.

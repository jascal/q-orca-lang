"""OpenAI LLM provider for Q-Orca."""
...
class OpenAIProvider(LLMProvider):
    def __init__(self, config: LLMProviderConfig):
        ...
        self.api_key = config.api_key or os.environ.get("ORCA_API_KEY") or os.environ.get("OPENAI_API_KEY", "")

        if not self.api_key:
            raise ValueError("API key is required — set ORCA_API_KEY (or OPENAI_API_KEY) in your environment")
    ...
    async def complete(self, request: LLMRequest) -> LLMResponse:
        ...
        # Build messages array — prepend system as first user message if present
        messages = []
        if system_message:

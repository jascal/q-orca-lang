...
# ── Generate Actions Skill ──────────────────────────────────────────────────────

class ActionScaffold(TypedDict):
    name: str
    signature: str
    parameters: list[str]
    return_type: str
...
class GenerateActionsResult(TypedDict, total=False):
    status: str  # "success" | "error"
    machine: str
    actions: list[ActionScaffold]
    scaffolds: dict[str, str]
    error: str | None


def generate_actions_skill(input: SkillInput, language: str = "python") -> GenerateActionsResult:
    """Generate action scaffolds for a Q-Orca machine."""
    try:
        source = _resolve_source(input)
        parsed = parse_q_orca_markdown(source)

        if not parsed.file.machines:
            return GenerateActionsResult(status="error", machine="unknown", actions=[], scaffolds={}, error="No machine found")
        ...
        for action in machine.actions:
            if language == "python":
                scaffolds[action.name] = _python_action_scaffold(action)
            elif language == "typescript":
                scaffolds[action.name] = _typescript_action_scaffold(action)
            else:
                scaffolds[action.name] = _python_action_scaffold(action)  # default to python

        actions = [
            ActionScaffold(
                name=a.name,
                signature=f"{a.name}({', '.join(a.parameters)}) -> {a.return_type}",
                parameters=a.parameters,
                return_type=a.return_type,
                has_effect=a.has_effect,
                effect_type=a.effect_type,
                context_used=[],  # would need guard analysis to determine
            )
            for a in machine.actions
        ]

        return GenerateActionsResult(status="success", machine=machine.name, actions=actions, scaffolds=scaffolds)
    except Exception as e:
        return GenerateActionsResult(status="error", machine="unknown", actions=[], scaffolds={}, error=str(e))


def _python_action_scaffold(action) -> str:
    if action.has_effect:
        return f'''# Action: {action.name}
# Effect type: {action.effect_type}
# Register via: bus.register_effect_handler("{action.effect_type}", {action.name})

async def {action.name}(effect):
    # effect.payload contains {{"action": "{action.name}", "context": {{...}}, "event": None}}
    ctx = effect.payload.get("context", {{}})
    # TODO: Implement quantum {action.name} effect
    return {{"status": "success", "data": {{}}}}
'''
    gate_info = ""
    if action.gate:
        gate_info = f"\n# Gate: {action.gate.kind} on qubits {action.gate.targets}"
        if action.gate.controls:
            gate_info += f" (controls: {action.gate.controls})"
    return f'''# Action: {action.name}{gate_info}
# Register via: machine.register_action("{action.name}", {action.name})

async def {action.name}(ctx: dict, event=None) -> dict:
    # TODO: Implement quantum {action.name} action
    qubits = ctx.get("qubits", [])
    # Apply quantum gate here
    return ctx
'''


def _typescript_action_scaffold(action) -> str:
    if action.has_effect:
        return f'''// Action: {action.name}
// Effect: {action.effect_type}

export async function {action.name}(effect: any): Promise<EffectResult> {{
  // TODO: Implement quantum {action.name} effect
  return {{ status: "success", data: {{}} }};
}}
'''
...
export async function {action.name}(ctx: Context): Promise<Context> {{
  // TODO: Implement quantum {action.name} action
  return ctx;
}}
'''

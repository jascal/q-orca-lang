...
def check_unitarity(machine: QMachineDef) -> QVerificationResult:
    ...
    for action in machine.actions:
        ...
        if action.gate.kind not in KNOWN_UNITARY_GATES:
            if action.gate.kind == "custom":
                errors.append(QVerificationError(
                    code="UNVERIFIED_UNITARITY",
                    message=f"Custom gate '{action.gate.custom_name or 'unknown'}' in action '{action.name}' cannot be statically verified as unitary",
                    severity="warning",
            ...
            for idx in action.gate.targets:
                if idx >= qubit_count:
                    errors.append(QVerificationError(
                        ...
                        message=f"Gate '{action.gate.kind}' in action '{action.name}' targets qubit {idx}, but machine only has {qubit_count} qubit(s)",
                        severity="error",
                        location={"action": action.name},
                    ))

            if action.gate.controls:
                for idx in action.gate.controls:
                    if idx >= qubit_count:
                        errors.append(QVerificationError(
                            code="QUBIT_INDEX_OUT_OF_RANGE",
                            message=f"Gate '{action.gate.kind}' in action '{action.name}' uses control qubit {idx}, but machine only has {qubit_count} qubit(s)",
                            severity="error",
                            location={"action": action.name},
                        ))

        if action.gate.controls and action.gate.targets:
            overlap = [c for c in action.gate.controls if c in action.gate.targets]
            if overlap:
                errors.append(QVerificationError(
                    code="CONTROL_TARGET_OVERLAP",
                    message=f"Gate '{action.gate.kind}' in action '{action.name}' has overlapping control and target qubits: {', '.join(str(x) for x in overlap)}",
                    severity="error",
                    location={"action": action.name},
                ))

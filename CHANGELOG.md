...
- **Stabilizer sampling — `compile_to_stim`** (`add-stabilizer-sampling`) — a Clifford machine can now be compiled to a runnable `stim.Circuit` and sampled. `q_orca.compiler.stabilizer.compile_to_stim` maps Clifford gates to Stim primitives, `measure(qs[i]) -> bits[j]` to `M` (tracking a bit→record map), and single-clause Pauli feedforward (`if bits[j] == 1: X/Y/Z(qs[k])`) to Stim's record-controlled `CX`/`CY`/`CZ rec[-N]` — e.g. `active-teleportation` compiles to `CX rec[-1] 2` / `CZ rec[-2] 2`, and teleporting `|0>` recovers the qubit exactly across 10k shots. `sample_stim_circuit` returns seeded outcome counts; `compile_to_qiskit_stabilizer` is a secondary `AerSimulator(method="stabilizer")` target. The compiler fails fast with a `StabilizerCompileError` on a non-Clifford machine, a non-Pauli / `== 0` / multi-clause feedforward, or an adaptive loop — rather than emitting a wrong circuit. Multi-clause QEC syndrome decoding (`if b0 and b1: …`) is a decoder concern (Stim detectors + PyMatching) and is a deferred follow-on, as are `MR` (no `reset` syntax yet) and the authored surface-code/repeated-bit-flip sampling examples.

- **Stabilizer (Clifford) fast-path verification backend** ([PR #121](../../pull/121), `add-stabilizer-backend`) — Stage-4b dynamic verification can now run Clifford-only machines on a Stim stabilizer tableau in polynomial time instead of the `O(2ⁿ)` state-vector path. A new Clifford classifier (`q_orca/compiler/stabilizer.py`) routes machines; the entanglement check (von Neumann entropy + Schmidt rank) is computed from the stabilizer check matrix via GF(2) rank (Fattal et al.) and matches the QuTiP verdict exactly. This unlocks QEC encoders/syndrome-extractors, graph states, and randomized benchmarking at hardware-relevant scale — a 30-qubit GHZ state, intractable for the state-vector path, verifies in milliseconds. Stim is an optional dependency (`pip install 'q-orca[stabilizer]'`). Backend selection via `--backend stim|stabilizer|auto|state-vector`, the `orca.yaml` `backend` key, or the `## assertion policy` `backend` field; forcing the stabilizer backend on a non-Clifford machine raises `NON_CLIFFORD_GATE_IN_STABILIZER_BACKEND` (fatal unless `stabilizer_fallback: state-vector`). See `docs/language/stabilizer-backend.md`. The sampling path (`compile_to_stim`), the Aer-stabilizer engine, and `q-orca run` acceleration are deferred follow-ons.
...
- **Default verification backend is now `auto`** ([PR #121](../../pull/121)) — `config.backend` defaults to `auto` (was `qutip`), so `q-orca verify` routes a Clifford machine to the stabilizer (`stim`) backend when available and any other to the state-vector path. Backward-compatible: non-Clifford machines behave exactly as before, programmatic `verify(machine)` still defaults to the QuTiP path, and `q-orca run`/`simulate` resolve `auto` to the state-vector simulator (the stabilizer backend is verification-only in this release). The `--json` verify report now shows the backend that actually ran (e.g. `stim`).

### Tests

- New `tests/test_stabilizer_backend.py` (47 cases) covers the Clifford classifier (examples + constructed gates + π/2-angle boundary), the stabilizer entanglement parity against QuTiP (product / Bell / GHZ), backend resolution (auto-routing, `state-vector`/`stabilizer` aliases, force/refuse with location, `stabilizer_fallback`), the `stabilizer_fallback` policy key, and a 30-qubit-GHZ stabilizer verification that is intractable for the state-vector path.
